...
export const targetsFormSchema = z
  .object({
    ...
    targetApi: z.preprocess((val) => emptyToDefault(val, DEFAULT_TARGETS.targetApi), z.number().min(0, "Target API minimal 0")),
    periodeKerjaAwal: z.preprocess((val) => emptyToDefault(val, DEFAULT_TARGETS.periodeKerjaAwal), z.number().int().min(1).max(12)),
    periodeKerjaAkhir: z.preprocess((val) => emptyToDefault(val, DEFAULT_TARGETS.periodeKerjaAkhir), z.number().int().min(1).max(12)),
  })
  .refine((data) => data.periodeKerjaAkhir >= data.periodeKerjaAwal, {
    message: "Periode kerja akhir harus lebih besar atau sama dengan periode awal",
    path: ["periodeKerjaAkhir"],
  });
